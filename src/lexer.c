...
Token lexer_next(Lexer *lexer) {
    ...
    if (!isspace(lexer->content[lexer->cursor]) && !is_symbol(lexer->content[lexer->cursor]) && lexer->content[lexer->cursor] != '(' && lexer->content[lexer->cursor] != ')') {
        while (lexer->cursor < lexer->content_len && !isspace(lexer->content[lexer->cursor]) && !is_symbol(lexer->content[lexer->cursor] && lexer->content[lexer->cursor] != '(' && lexer->content[lexer->cursor] != ')')) {
            lexer->cursor += 1;
            token.text_len += 1;
